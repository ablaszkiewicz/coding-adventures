...
export class Camera {
  ...
  public render(
    ...
    for (
      let j = options.areaToRender.yMin;
      j < options.areaToRender.yMax;
      j++
    ) {
      for (
        let i = options.areaToRender.xMin;
        i < options.areaToRender.xMax;
        i++
      ) {
        ...
        for (let sample = 0; sample < options.samples; sample++) {
          ...
          color = color.add(this.rayColor(ray, world, options.bounces));
        }
        ...
        const percentageDone =
          ((i * IMAGE_HEIGHT + j) / (IMAGE_WIDTH * IMAGE_HEIGHT)) * 100;

        if (progressCallback && percentageDone - lastReportedPercentage > 1) {
